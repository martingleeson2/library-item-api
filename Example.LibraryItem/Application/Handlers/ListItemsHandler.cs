...
namespace Example.LibraryItem.Application.Handlers
{
    public class ListItemsHandler(LibraryDbContext db, ILogger<ListItemsHandler> logger) : IListItemsQueryHandler
    {
        public async Task<ItemListResponseDto> HandleAsync(ListItemsQuery q, CancellationToken ct = default)
        {
            ...
            if (!string.IsNullOrWhiteSpace(q.title)) query = query.Where(i => i.Title.Contains(q.title));
            if (!string.IsNullOrWhiteSpace(q.author)) query = query.Where(i => i.Author != null && i.Author.Contains(q.author));
            if (!string.IsNullOrWhiteSpace(q.isbn)) query = query.Where(i => i.Isbn == q.isbn);
            if (q.item_type.HasValue) query = query.Where(i => i.ItemType == q.item_type);
            if (q.status.HasValue) query = query.Where(i => i.Status == q.status);
            if (!string.IsNullOrWhiteSpace(q.collection)) query = query.Where(i => i.Collection == q.collection);
            if (q.location_floor.HasValue) query = query.Where(i => i.Location.Floor == q.location_floor);
            if (!string.IsNullOrWhiteSpace(q.location_section)) query = query.Where(i => i.Location.Section == q.location_section);
            if (!string.IsNullOrWhiteSpace(q.call_number)) query = query.Where(i => i.CallNumber.Contains(q.call_number));
            if (q.publication_year_from.HasValue) query = query.Where(i => i.PublicationDate != null && i.PublicationDate.Value.Year >= q.publication_year_from);
            if (q.publication_year_to.HasValue) query = query.Where(i => i.PublicationDate != null && i.PublicationDate.Value.Year <= q.publication_year_to);

            query = (q.sort_by, q.sort_order) switch
            {
